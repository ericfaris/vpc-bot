...
module.exports = {
  ...
  description: 'Search in High Scores for table name (MANAGE_GUILD)',
  minArgs: 1,
  expectedArgs: '<searchterm>',
  callback: async ({ args, client, channel, interaction, instance }) => {
    ...
    if (channel.name !== process.env.COMPETITION_CHANNEL_NAME) {
      ...
      const tables = (await mongoHelper.find({tableName:{$regex:'.*' + searchTerm + '.*', $options: 'i'}}, 'tables'))
        .sort((a, b) => a.tableName - b.tableName);

      retVal = "data returned...."
    }
